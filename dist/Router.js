...
class HandlerStore extends Array {
    ...
    shellSort() {
        ...
        while (i > 0) {
            for (j = i; j < this.length; j++) {
                let k = j, t = this[j];
                while (k >= i && this[k - i] > t) this[k] = this[k -= i];
                this[k] = t;
            }
            i = Number(i === 2) || ~~(i * 5 / 11);
        }
    ...
    quickSort() {
        const partition = (arr, left, right, ) => {
            ...
            while (left <= right) {
                while (f(arr[left], pivot) < 0) left++;
                while (f(arr[right], pivot) > 0) right--;
                left <= right &&
        ...
        const quicksort = (arr, left, right, f) => {
            const mid = partition(arr, left, right, f);
            (left < mid - 1) && quicksort(arr, left, mid - 1, f);
            (right > mid) && quicksort(arr, mid, right, f);
        }
        quicksort(this, 0, this.length - 1, f);
    }

    add(handler) {
        const index = this.findIndex(h => handler.priority < h.priority);
        if (index === -1) {
...
class Router {
    constructor(pattern) {
        Object.defineProperties(this, {
            ...
            readHandlers: { value: [] },
            updateHandlers: { value: [] },
            deleteHandlers: { value: [] }
        })
    }
